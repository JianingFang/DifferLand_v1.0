...
et_baseline = ncread("./nc/daily_US-Var_baseline_16.nc", "ET")
et_baseline = et_baseline(start_idx:end)

et_default = ncread("./nc/daily_US-Var_default_36.nc", "ET") 
et_default = et_default(start_idx:end)

et_paw = ncread("./nc/daily_US-Var_nn_paw_12.nc", "ET") 
et_paw = et_paw(start_idx:end)

et_whole = ncread("./nc/daily_US-Var_nn_whole_4.nc", "ET") 
et_whole = et_whole(start_idx:end)

et_whole_no_lai = ncread("./nc/daily_US-Var_nn_whole_no_lai_21.nc", "ET")
et_whole_no_lai = et_whole_no_lai(start_idx:end)

et_h_acm = ncread("./nc/daily_US-Var_gpp_acm_et_nn_33.nc", "ET") 
et_h_acm = et_h_acm(start_idx:end)

et_obs = ncread("./nc/US-Var.nc", "ET")
et_obs = et_obs(start_idx:end)
et_obs = fillmissing(et_obs, "linear")
...
colormap("default")
subplot(2, 2, 1)
[wcoh_baseline,~,period_baseline,coi_baseline]  = wcoherence(et_baseline, et_obs, days(1), 'phasedisplaythreshold',0.7);
wcoherence(et_baseline, et_obs, days(1), 'phasedisplaythreshold',0.7);
hold on 
...
subplot(2, 2, 2)
[wcoh_paw,~,period_paw,coi_paw]  = wcoherence(et_paw, et_obs, days(1), 'phasedisplaythreshold',0.7)
wcoherence(et_paw, et_obs, days(1), 'phasedisplaythreshold',0.7)
hold on 
...
subplot(2, 2, 3)
[wcoh_h_acm,~,period_h_acm,coi_h_acm]  = wcoherence(et_h_acm, et_obs, days(1), 'phasedisplaythreshold',0.7);
wcoherence(et_h_acm, et_obs, days(1), 'phasedisplaythreshold',0.7);
hold on 
