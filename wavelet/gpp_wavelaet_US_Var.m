...
gpp_baseline = ncread("./nc/daily_US-Var_baseline_16.nc", "gpp")
gpp_baseline = gpp_baseline(start_idx:end)

gpp_default = ncread("./nc/daily_US-Var_default_36.nc", "gpp") 
gpp_default = gpp_default(start_idx:end)

gpp_paw = ncread("./nc/daily_US-Var_nn_paw_12.nc", "gpp") 
gpp_paw = gpp_paw(start_idx:end)

gpp_whole = ncread("./nc/daily_US-Var_nn_whole_4.nc", "gpp") 
gpp_whole = gpp_whole(start_idx:end)

gpp_whole_no_lai = ncread("./nc/daily_US-Var_nn_whole_no_lai_21.nc", "gpp")
gpp_whole_no_lai = gpp_whole_no_lai(start_idx:end)

gpp_h_acm = ncread("./nc/daily_US-Var_gpp_acm_et_nn_33.nc", "gpp") 
gpp_h_acm = gpp_h_acm(start_idx:end)

gpp_obs = ncread("./nc/US-Var.nc", "GPP")
gpp_obs = gpp_obs(start_idx:end)
gpp_obs = fillmissing(gpp_obs, "linear")

DOY = ncread("./US-Var_driver_daily_v4.nc", "DOY")
DOY = DOY(start_idx:end)
set(gcf,'color','w');
colormap("default")
subplot(2, 2, 1)
[wcoh_baseline,~,period_baseline,coi_baseline]  = wcoherence(gpp_baseline, gpp_obs, days(1), 'phasedisplaythreshold',0.7);
wcoherence(gpp_baseline, gpp_obs, days(1), 'phasedisplaythreshold',0.7);
hold on 
...
subplot(2, 2, 2)
[wcoh_paw,~,period_paw,coi_paw]  = wcoherence(gpp_paw, gpp_obs, days(1), 'phasedisplaythreshold',0.7)
wcoherence(gpp_paw, gpp_obs, days(1), 'phasedisplaythreshold',0.7)
hold on 
...
subplot(2, 2, 3)
[wcoh_h_acm,~,period_h_acm,coi_h_acm]  = wcoherence(gpp_h_acm, gpp_obs, days(1), 'phasedisplaythreshold',0.7);
wcoherence(gpp_h_acm, gpp_obs, days(1), 'phasedisplaythreshold',0.7);
hold on 
